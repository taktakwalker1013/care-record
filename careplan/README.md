...
careplan/
  ...
  knowledge/     型テンプレート（type1A/1B/2A・制度知識）。LLM呼び出しに同梱
...
**型テンプレートの同梱**：LLM実行器は段に応じて `knowledge/` の型テンプレートをシステムに同梱します。
型判定（01）には3型の「適用対象」、ニーズ抽出（02）・原案作成（03）には選択された型の全文を渡し、
サービス組合せ・役割分担・記載例・注意点を根拠にさせます（`generic` は同梱なし）。
